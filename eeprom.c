...
void STMFLASH_Write(u32 WriteAddr,u16 *pBuffer,u16 NumToWrite)	
{
	...
	while(1)
	{
		...
		if(i<sec_remain)
		{
			FLASH_ErasePage(sec_addr*STM_SECTOR_SIZE+STM32_FLASH_BASE);
		}
		for(i=0;i<sec_remain;i++)
		{
			FLASH_ProgramHalfWord(WriteAddr+2*i,pBuffer[i]);
		}
		if(NumToWrite==sec_remain)break;
		else
		{
			sec_addr++;
			sec_off=0;
			pBuffer+=sec_remain;
			WriteAddr+=sec_remain*2;
			NumToWrite-=sec_remain;
			if(NumToWrite>STM_SECTOR_SIZE/2)sec_remain=STM_SECTOR_SIZE/2;
			else sec_remain=NumToWrite;
		}
	}
	FLASH_Lock();//上锁
}
